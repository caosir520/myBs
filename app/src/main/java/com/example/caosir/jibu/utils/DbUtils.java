package com.example.caosir.jibu.utils;
...
public class DbUtils {
    ...
    public static <T> List<T> getQueryByWhere(Class<T> cla, String field, String[] value) {
        if("name".equals(field)){
            ...
            LogUtil.d(name);
            return liteOrm.<T>query(new QueryBuilder(cla).where(field + "=?",value).whereAppendAnd().whereAppend("user_name=?",new String[]{SettingUtils.getSharedPreferences(MyApplication.getContext(), Constant.USER_NAME,"")}));
        }
    }
    ...
    //升序排列
    public static <T>List<T> getQueryByTop(Class<T> cla, String field) {
        return liteOrm.<T>query(new QueryBuilder(cla).appendOrderAscBy(field).where("user_name=?",new String[]{SettingUtils.getSharedPreferences(MyApplication.getContext(), Constant.USER_NAME,"")}));
    }
