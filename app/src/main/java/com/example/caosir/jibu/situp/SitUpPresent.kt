...
class SitUpPresent : BasePresenter<SitUpPresent.UI>() {
    ...
    fun getMostLast() {
        val list = DbUtils.getQueryByTop(Exercise::class.java, "set_up")
        if (list.size == 0) {
            ui.showLast(0)
            ui.showMost(0)
        } else {
            ui.showLast(list[0].setUp)
            ui.showMost(list[list.size - 1].setUp)
        }
    }
