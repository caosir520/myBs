...
public class StepInAcceleration extends StepMode {
    ...
    public void detectorNewStep(float values) {
        if (gravityOld == 0) {
            ...
            if (DetectorPeak(values, gravityOld)) {
                ...
                //条件为：这次坡峰的时间和上次差距大于200ms小于2000ms，并且峰值-谷值大于初始阀值
    ...
    public float Peak_Valley_Thread(float value) {
        ...
        if (tempCount < valueNum) {
            ...
            tempThread = averageValue(tempValue, valueNum); //生成动态法制
            for (int i = 1; i < valueNum; i++) {
                tempValue[i - 1] = tempValue[i]; //去除最早的那个差值，并整理数组
            }
            tempValue[valueNum - 1] = value; //记录这次的差值
        }
        return tempThread; //用于下次判断，
    ...
    public float averageValue(float value[], int n) {
        float ave = 0;
        for (int i = 0; i < n; i++) {
            ave += value[i]; //计算4次法制的和
        }
        ave = ave / valueNum; //计算平均值
        if (ave >= 8) {
//            Log.v(TAG, "超过8");
            ave = (float) 4.3;
        } else if (ave >= 7 && ave < 8) {
